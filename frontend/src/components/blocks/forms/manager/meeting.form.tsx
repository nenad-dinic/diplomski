...
export interface ManagerMeetingFormData {
...
interface ManagerMeetingFormProps {
    ...
    buildingId ?: number;
    managerId : number;
    onSubmit?: (values : ManagerMeetingFormData) => void;

}

export default function ManagerMeetingForm(props : ManagerMeetingFormProps) {
    ...
    const form = useForm<ManagerMeetingFormData>({
    ...
    function onSubmit(values : ManagerMeetingFormData) {
    ...
    useEffect(() => {
        ...
        if(props.meeting != null) {
            form.reset({
                buildingId: props.buildingId ?? props.meeting.buildingId,
        ...
        <form className="flex flex-col gap-2" onSubmit={form.handleSubmit(onSubmit)}>
            <FormField control={form.control} name="buildingId" render={({field}) => (
                <FormItem>
                    ...
                    <Combobox disabled={props.buildingId != undefined || props.meeting != null} data={buildings.map(b => ({label: b.address, value: b.id}))} value={field.value} onChange={v => form.setValue(field.name, v)}></Combobox>
                    <FormMessage />
                </FormItem>
            )} />
