...
export interface ManagerPollFormData {
...
interface ManagerPollFormProps {

    poll ?: Poll;
    buildingId ?: number;
    managerId : number;
    onSubmit?: (values : ManagerPollFormData) => void;

}

export default function ManagerPollForm(props : ManagerPollFormProps) {
    ...
    const form = useForm<ManagerPollFormData>({
    ...
    async function getBuildings() {
        ...
        const building = await BuildingService.getBuildingsByManager(props.managerId, "", 1, 1000);
    ...
    function onSubmit(values : ManagerPollFormData) {
    ...
    useEffect(() => {
        ...
        if(props.poll != null) {
            form.reset({
                buildingId: props.buildingId ?? props.poll.buildingId,
        ...
        <form className="flex flex-col gap-2" onSubmit={form.handleSubmit(onSubmit)}>
            <FormField control={form.control} name="buildingId" render={({field}) => (
                <FormItem>
                    ...
                    <Combobox disabled={props.buildingId != undefined || props.poll != null} data={buildings.map(b => ({label: b.address, value: b.id}))} value={field.value} onChange={v => form.setValue(field.name, v)}></Combobox>
                    <FormMessage />
                </FormItem>
            )} />
